...
class SensorsClientTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sensors = {'enter_door_switch' : {'description': 'Enter  Door', 'device': 'RelaySwitch', 'args': {'gpio': 'GPIO', 'invert': False, 'channel': CONF_DOOR_ENTER_GPIO}, 'name': 'enter_door_relay'},
    ...
    def testSetFunction(self):
        ...
        self.setChannelFunction(GPIO().pins[12], 'IN')
        self.setChannelFunction(GPIO().pins[12], 'OUT')

    def testSetValue(self):
        debug('testSetValue')
        self.setChannelFunction(GPIO().pins[12], 'OUT')
        self.setChannelValue(GPIO().pins[12], 1)
        self.setChannelValue(GPIO().pins[12], 0)
    ...
    def testSensorInfo(self):
        ...
        actuator_channel = GPIO().pins[13]
        light_switch_channel = GPIO().pins[19]
        sensors = {'actuator' : {'description': 'Digital Output', 'device': 'DigitalActuator', 'args': {'gpio': 'GPIO', 'invert': False, 'channel': actuator_channel}, 'name': 'test_actuator1'},
                   'light_switch' : {'description': 'Light Switch', 'device': 'LightSwitch', 'args': {'gpio': 'GPIO', 'invert': True, 'channel': light_switch_channel}, 'name': 'test_light_switch1'}
                #    'MCP3004' : {'description': 'MCP3004', 'device': 'MCP3004', 'args': {'chip': '0'}, 'name': 'test_MCP3004'},
                #    'distance' : {'description': 'Analog Distance Sensor', 'device': 'DistanceSensor', 'args': {'adc': 'test_MCP3004', 'channel': 0}, 'name': 'test_distance'}
                  }
    ...
    def testListSensors(self):
        ...
        info(deviceNames)
    def testAddSensors(self):
        info('testAddSensors')
        enter_door_channel = 13
        info("actuator_channel {}".format(enter_door_channel))
        exit_door_channel = 19
        info("light_switch_channel {}".format(exit_door_channel))
        sensors = {'enter_door_switch' : {'description': 'Enter  Door', 'device': 'RelaySwitch', 'args': {'gpio': 'GPIO', 'invert': False, 'channel': enter_door_channel}, 'name': 'enter_door_relay'},
...
if __name__ == '__main__':
    ...
    SensorsClientTest("testSetGPIOValue"),
    ]
